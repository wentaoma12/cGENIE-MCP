...
CONTAINS
  ...
  SUBROUTINE sub_def_tracerrelationships()
    ...
    conv_RDOM_POM(:,:) = 0.0
    conv_RDOM_POM(is_POC,io_RDOM_C)               = 1.0
    conv_RDOM_POM(is_POC_13C,io_RDOM_C_13C)       = 1.0
    conv_RDOM_POM(is_POC_14C,io_RDOM_C_14C)       = 1.0
    conv_RDOM_POM(is_PON,io_RDOM_N)               = 1.0
    conv_RDOM_POM(is_PON_15N,io_RDOM_N_15N)       = 1.0
    conv_RDOM_POM(is_POP,io_RDOM_P)               = 1.0
    conv_RDOM_POM(is_POCd,io_RDOM_Cd)             = 1.0
    conv_RDOM_POM(is_POCd_114Cd,io_RDOM_Cd_114Cd) = 1.0
    conv_RDOM_POM(is_POFe,io_RDOM_Fe)             = 1.0
    conv_RDOM_POM(is_POFe_56Fe,io_RDOM_Fe_56Fe)   = 1.0
    conv_RDOM_POM(is_POI,io_RDOM_I)               = 1.0   
    ! convert POM -> URDOM
    ! conv_POM_URDOM(:,:) = conv_POM_DOM(:,:)
    conv_POM_URDOM(:,:) = 0.0
    conv_POM_URDOM(io_URDOM_C,is_POC)             = 1.0
    conv_POM_URDOM(io_URDOM_N,is_PON)             = 1.0
    conv_POM_URDOM(io_URDOM_P,is_POP)             = 1.0
    conv_POM_URDOM(io_URDOM_C_13C,is_POC_13C)     = 1.0
    conv_POM_URDOM(io_URDOM_C_14C,is_POC_14C)     = 1.0
    ! convert URDOM -> POM
    ! conv_URDOM_POM(:,:) = conv_DOM_POM(:,:)
    conv_URDOM_POM(:,:) = 0.0
    conv_URDOM_POM(is_POC,io_URDOM_C)             = 1.0
    conv_URDOM_POM(is_PON,io_URDOM_N)             = 1.0 
    conv_URDOM_POM(is_POP,io_URDOM_P)             = 1.0
    conv_URDOM_POM(is_POC_13C,io_URDOM_C_13C)     = 1.0
    conv_URDOM_POM(is_POC_14C,io_URDOM_C_14C)     = 1.0
  ...
  SUBROUTINE sub_calc_tracerrelationships_i()
    ...
    ! identify the indices of all non-zero transformation values in the conversion array for URDOM -> POM
    do io=1,n_ocn
       loc_tot_i = 0
       do is=1,n_sed
          if (abs(conv_URDOM_POM(is,io)) > const_real_nullsmall) then 
             loc_tot_i = loc_tot_i + 1
             conv_URDOM_POM_i(loc_tot_i,io) = is 
          end if
       end do
       conv_URDOM_POM_i(0,io) = loc_tot_i
    end do
    ! identify the indices of all non-zero transformation values in the conversion array for POM -> URDOM 
    do is=1,n_sed
       loc_tot_i = 0
       do io=1,n_ocn
          if (abs(conv_POM_URDOM(io,is)) > const_real_nullsmall) then 
             loc_tot_i = loc_tot_i + 1
             conv_POM_URDOM_i(loc_tot_i,is) = io 
          end if
       end do
       conv_POM_URDOM_i(0,is) = loc_tot_i
    end do
