...
CONTAINS
  ...
  SUBROUTINE sub_load_goin_biogem(dum_dtyr)
    ...
    if (ctrl_debug_init > 0) then
       ...
       print*,'Production fraction of ultra-RDOM                   : ',par_bio_red_URDOMfrac
       ...
       print*,'P:C fractionation during POM->URDOM production      : ',par_bio_red_rP_POM_URDOM
       print*,'N:C fractionation during POM->URDOM production      : ',par_bio_red_rN_POM_URDOM
       ...
       print*,'URDOM lifetime (yrs)                                : ',par_bio_remin_URDOMlifetime
       print*,'RDOM degradation by (surface) photolysis only?      : ',ctrl_bio_remin_RDOM_photolysis
       print*,'URDOM degradation by (surface) photolysis only?     : ',ctrl_bio_remin_URDOM_photolysis
...
!!!
    ...
    ! -------------------------------------------------------- ! POM -> URDOM 
    conv_lP_lRD(:,:)   =  fun_conv_sedocn2lslo(conv_POM_URDOM(:,:))
    conv_lP_lRD_i(:,:) =  fun_conv_sedocn2lslo_i(conv_POM_URDOM_i(:,:))
    ! -------------------------------------------------------- ! URDOM -> POM
    conv_lRD_lP(:,:)   =  fun_conv_ocnsed2lols(conv_URDOM_POM(:,:))
    conv_lRD_lP_i(:,:) =  fun_conv_ocnsed2lols_i(conv_URDOM_POM_i(:,:))
    ! -------------------------------------------------------- !
    ! END
    ! -------------------------------------------------------- !
  ...
  SUBROUTINE sub_init_int_timeslice()
    ...
    int_diag_production_timeslice(:,:,:,:) = 0.0
...
!!$       ctrl_data_save_sig_diag_geochem = .true.
    ...
    IF ( &
         ...
         & .OR. &
         & (force_restore_ocn_select(io_RDOM_C_13C) .AND. force_flux_atm_select(ia_pCO2_13C)) &
         & .OR. &
         & (force_restore_ocn_select(io_URDOM_C_13C) .AND. force_flux_atm_select(ia_pCO2_13C)) &
         & .OR. &
