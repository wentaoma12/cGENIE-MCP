...
!!!integer::nthreads,thread_id
  ...
  if_go: IF (par_misc_t_go) THEN
     ...
     DO i=1,n_i
        DO j=1,n_j
           ...
           IF (n_k >= loc_k1) THEN
              ...
              IF ( &
                   & (force_restore_ocn_select(io_DIC_13C) .OR. force_restore_ocn_select(io_DOM_C_13C) .OR. force_restore_ocn_select(io_RDOM_C_13C) .OR. force_restore_ocn_select(io_URDOM_C_13C)) &
                   &  .AND. &
                 ...
                 elseif (force_restore_ocn_select(io_DOM_C_13C)) then
                    ...
                    loc_force_actual_d13C = loc_force_actual_d13C + loc_k_icefree*&
                         ...
                         & loc_k_tot_icefree
                 elseif (force_restore_ocn_select(io_RDOM_C_13C)) then
                    loc_standard = const_standards(ocn_type(io_RDOM_C_13C))
                    loc_force_actual_d13C = loc_force_actual_d13C + loc_k_icefree*&
                         & fun_calc_isotope_delta &
                         & (ocn(io_RDOM_C,i,j,n_k),ocn(io_RDOM_C_13C,i,j,n_k),loc_standard,.FALSE.,const_real_null)/&
                         & loc_k_tot_icefree
                 elseif (force_restore_ocn_select(io_URDOM_C_13C)) then
                    loc_standard = const_standards(ocn_type(io_URDOM_C_13C))
                    loc_force_actual_d13C = loc_force_actual_d13C + loc_k_icefree*&
                         & fun_calc_isotope_delta &
                         & (ocn(io_URDOM_C,i,j,n_k),ocn(io_URDOM_C_13C,i,j,n_k),loc_standard,.FALSE.,const_real_null)/&
                         & loc_k_tot_icefree
                 end if
              end IF
              ! calc mean Ca d44Ca
     ...
     !        & fun_calc_isotope_delta(                                          &
     !        & SUM(phys_ocn(ipo_M,:,:,:)*ocn(io_URDOM_C,:,:,:))*loc_ocn_rtot_M,     &
     !        & SUM(phys_ocn(ipo_M,:,:,:)*ocn(io_URDOM_C_13C,:,:,:))*loc_ocn_rtot_M, &
     !        & loc_standard,.FALSE.,const_real_null)
     !print*,loc_force_actual_d13C
     !read(*,*)
     ! calc mean DIC d13C
...
!!$                          id = fun_find_str_i(trim(loc_string),string_diag_redox)
     ...
     do n=1,n_vocn
        ...
        call sub_box_remin_part(loc_dtyr,vocn(n),vphys_ocn(n),vbio_part(n),vbio_remin(n))
          ...
          ! int_diag_production_timeslice(:,:,:,:) = int_diag_production_timeslice(:,:,:,:) + diag_production(:,:,:,:)
              ...
              elseif (force_restore_ocn_select(io_ALK) .AND. force_flux_ocn_select(io_ALK)) THEN
                 ...
                 elseif ( &
                    ...
                    elseif (force_restore_ocn_select(io_RDOM_C_13C)) then
                       ...
                       if (par_force_point_i*par_force_point_j /= 0) then
                          loc_delta_actual = fun_calc_isotope_delta( &
                               & ocn(io_RDOM_C,par_force_point_i,par_force_point_j,n_k), &
                               & ocn(io_RDOM_C_13C,par_force_point_i,par_force_point_j,n_k), &
                               & loc_standard,.FALSE.,const_real_null &
                               & )
                          loc_delta_target = fun_calc_isotope_delta( &
                               & force_restore_locn(io2l(io_RDOM_C),par_force_point_i,par_force_point_j,n_k), &
                               & force_restore_locn(io2l(io_RDOM_C_13C),par_force_point_i,par_force_point_j,n_k), &
                               & loc_standard,.FALSE.,const_real_null &
                               & )
                       else
                          loc_delta_actual = loc_force_actual_d13C
                          loc_delta_target = fun_calc_isotope_delta( &
                               & force_restore_locn(io2l(io_RDOM_C),i,j,n_k),force_restore_locn(io2l(io_RDOM_C_13C),i,j,n_k), &
                               & loc_standard,.FALSE.,const_real_null &
                               & )
                       end if
                    elseif (force_restore_ocn_select(io_URDOM_C_13C)) then
                       ...
                       if (par_force_point_i*par_force_point_j /= 0) then
                          loc_delta_actual = fun_calc_isotope_delta( &
                               & ocn(io_URDOM_C,par_force_point_i,par_force_point_j,n_k), &
                               & ocn(io_URDOM_C_13C,par_force_point_i,par_force_point_j,n_k), &
                               & loc_standard,.FALSE.,const_real_null &
                               & )
                          loc_delta_target = fun_calc_isotope_delta( &
                               & force_restore_locn(io2l(io_URDOM_C),par_force_point_i,par_force_point_j,n_k), &
                               & force_restore_locn(io2l(io_URDOM_C_13C),par_force_point_i,par_force_point_j,n_k), &
                               & loc_standard,.FALSE.,const_real_null &
                               & )
                       else
                          loc_delta_actual = loc_force_actual_d13C
                          loc_delta_target = fun_calc_isotope_delta( &
                               & force_restore_locn(io2l(io_URDOM_C),i,j,n_k),force_restore_locn(io2l(io_URDOM_C_13C),i,j,n_k), &
                               & loc_standard,.FALSE.,const_real_null &
                               & )
                       end if
                    end if
                    ...
                    IF (force_flux_ocn_select(io_DIC_13C)) THEN
                       ...
                       if (force_restore_ocn_select(io_RDOM_C_13C)) then
                          locijk_focn(io_RDOM_C,i,j,n_k)     = 0.0
                          locijk_focn(io_RDOM_C_13C,i,j,n_k) = 0.0
                       end if
                       if (force_restore_ocn_select(io_URDOM_C_13C)) then
                          locijk_focn(io_URDOM_C,i,j,n_k)     = 0.0
                          locijk_focn(io_URDOM_C_13C,i,j,n_k) = 0.0
                       end if
  ...
  if (ocn_select(io_URDOM_C)) then 
     DO l=3,n_l_ocn
        ...
        do loc_m=1,loc_tot_m
           loc_tmp_M = sum(phys_ocn(ipo_M,:,:,n_k)*dum_egbg_sfcremin(io,:,:,n_k))
           is = conv_DOM_POM_i(loc_m,io)
           loc_DOM_M(is) = loc_DOM_M(is) + conv_URDOM_POM_i(is,io)*loc_tmp_M
        end do
     end do
  end if
  ! ---------------------------------------------------------- ! POM
