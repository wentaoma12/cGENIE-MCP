...
! *************************************************************************************************
! latest modify: Yuxian Lai and Wentao Ma 2025/06/17
...
CONTAINS
  ...
  SUBROUTINE sub_calc_bio_uptake(dum_i,dum_j,dum_k1,dum_dt)
    ...
    real,dimension(n_sed,n_k)::loc_bio_part_URDOM                       !
    ...
    real::loc_kCd
    real::loc_TC
    real::loc_bio_red_DOMfrac,loc_bio_red_RDOMfrac,loc_bio_red_URDOMfrac,loc_bio_red_DOMtotal !
    real::loc_PPoverZeu,loc_rPOC
    real::loc_r_POM_DOM,loc_r_POM_RDOM,loc_r_POM_URDOM                                  !
    ...
    loc_bio_part_URDOM(:,:) = 0.0
    loc_k_mld = dum_k1
    loc_bio_red_DOMfrac = 0.0
    loc_bio_red_RDOMfrac = 0.0
    loc_bio_red_URDOMfrac = 0.0
...
!!$          if (sed_select(is_POFe_56Fe)) loc_FeT_56Fe = ocn(io_Fe_56Fe,dum_i,dum_j,n_k)
    ...
    case ('dunne')
       ...
       ! set DOMfrac
       ! NOTE: for now, ignore URDOM creation via primary production ...
       loc_bio_red_DOMfrac  = (1.0 - loc_rPOC)*0.9599
       loc_bio_red_RDOMfrac = (1.0 - loc_rPOC)*0.04
       loc_bio_red_URDOMfrac = (1.0 - loc_rPOC)*0.0001
    case ('simple')
       ...
       loc_bio_red_DOMfrac  = (1.0 - loc_rPOC)*(9.0/11.0)
       loc_bio_red_RDOMfrac = (1.0 - loc_rPOC)*(2.0/11.0)
       loc_bio_red_URDOMfrac = 0.0
    case ('DEFAULT')
       loc_bio_red_DOMfrac  = par_bio_red_DOMfrac
       loc_bio_red_RDOMfrac = par_bio_red_RDOMfrac
       loc_bio_red_URDOMfrac = par_bio_red_URDOMfrac
    case default
    ...
    ! *** ADJUST FOR TOTAL DOM + RDOM + URDOM ******************************************************************************************** !
    ! check for total DOM fraction exceeding 1.0 and re-scale (proportionally and to sum to 1.0)
    loc_bio_red_DOMtotal = loc_bio_red_DOMfrac + loc_bio_red_RDOMfrac + loc_bio_red_URDOMfrac
    if (loc_bio_red_DOMtotal > 1.0) then
       loc_bio_red_DOMfrac = loc_bio_red_DOMfrac/loc_bio_red_DOMtotal
       loc_bio_red_RDOMfrac = loc_bio_red_RDOMfrac/loc_bio_red_DOMtotal
       loc_bio_red_URDOMfrac = 1.0 - loc_bio_red_DOMfrac - loc_bio_red_RDOMfrac
       loc_bio_red_DOMtotal = 1.0
    end if
    ...
    diag_production(idiag_production_PP,dum_i,dum_j,n_k) = bio_part_red(is_POP,is_POC,dum_i,dum_j) * loc_dPO4
    ...
    DO l=1,n_l_sed
       ...
       do loc_i=1,loc_tot_i
          ...
          diag_production(idiag_production_LDOMpp,dum_i,dum_j,loc_k_mld:n_k) = loc_r_POM_DOM*loc_bio_red_DOMfrac*bio_part_red(is_POP,is_POC,dum_i,dum_j)*loc_dPO4
       end do
       ...
       do loc_i=1,loc_tot_i
          ...
          diag_production(idiag_production_RDOMpp,dum_i,dum_j,loc_k_mld:n_k) = loc_r_POM_RDOM*loc_bio_red_RDOMfrac*bio_part_red(is_POP,is_POC,dum_i,dum_j)*loc_dPO4
       end do
       ! create URDOM fraction
       loc_tot_i = conv_POM_URDOM_i(0,is)
       do loc_i=1,loc_tot_i
          io = conv_POM_URDOM_i(loc_i,is)
          ! set POM->DOM conversion modifier
          select case (ocn_dep(io))
          case (io_URDOM_P)
             loc_r_POM_URDOM = par_bio_red_rP_POM_URDOM
          case (io_URDOM_N)
             loc_r_POM_URDOM = par_bio_red_rN_POM_URDOM
          case default
             loc_r_POM_URDOM = 1.0
          end select
          ! calculate decrease in particulate fraction
          loc_bio_part_URDOM(is,loc_k_mld:n_k) = loc_r_POM_URDOM*loc_bio_red_URDOMfrac*bio_part(is,dum_i,dum_j,loc_k_mld:n_k)
          ! create (and add) dissolved tracers
          bio_remin(io,dum_i,dum_j,loc_k_mld:n_k) = bio_remin(io,dum_i,dum_j,loc_k_mld:n_k) + loc_bio_part_URDOM(is,loc_k_mld:n_k)
          diag_production(idiag_production_URDOMpp,dum_i,dum_j,loc_k_mld:n_k) = loc_r_POM_URDOM*loc_bio_red_URDOMfrac*bio_part_red(is_POP,is_POC,dum_i,dum_j)*loc_dPO4
       end do
       ! save total DOM fraction [NOTE: not a global mean ... just this (i,j) location ...]
       if ((loc_r_POM_DOM + loc_r_POM_RDOM + loc_r_POM_URDOM) > const_real_nullsmall) then
          int_fracdom(is) = loc_r_POM_DOM*loc_bio_red_DOMfrac + loc_r_POM_RDOM*loc_bio_red_RDOMfrac + loc_r_POM_URDOM*loc_bio_red_URDOMfrac
       else
          int_fracdom(is) = 0.0
       end if
    end do
    ...
    bio_part(:,dum_i,dum_j,loc_k_mld:n_k) = bio_part(:,dum_i,dum_j,loc_k_mld:n_k) - &
         (loc_bio_part_DOM(:,loc_k_mld:n_k) + loc_bio_part_RDOM(:,loc_k_mld:n_k) + loc_bio_part_URDOM(:,loc_k_mld:n_k)) 
  ...
  SUBROUTINE sub_box_remin_DOM(dum_vocn,dum_vbio_remin,dum_dtyr)
    ...
    real::tmp_bio_remin
    real::loc_bio_remin_DOMratio,loc_bio_remin_RDOMratio, loc_bio_remin_URDOMratio                !
    real::loc_bio_remin_DOMlifetime, loc_bio_remin_RDOMlifetime, loc_bio_remin_URDOMlifetime
    real::loc_intI                                                      ! local integrated insolation
    real,dimension(n_l_ocn,n_k)::loc_vbio_remin                         !
    real,dimension(n_sed,n_k)::loc_bio_part                             !
    real,dimension(n_sed,n_k)::loc_bio_part_URDOM                       !
    integer::loc_i,loc_j,loc_k1,lurdom
    integer::loc_k_mld
...
!!$    allocate(loc_diag_redox(n_diag_redox,n_k),STAT=alloc_error)
    ...
    DO k=n_k,loc_k1,-1
       ...
       ! calculate URDOM lifetime modifier
       if (ctrl_bio_remin_URDOM_photolysis) then
          ! restrict photolysis to the surface (n = n_k) layer (otherwise, set a 'high' (effectively infinite) lifetime)
          if (k == n_k) then
             If (phys_ocn(ipo_Dbot,loc_i,loc_j,n_k) >= phys_ocnatm(ipoa_mld,loc_i,loc_j)) then
                ! ml entirely within uppermost (surface) cell
             ...
             if (par_bio_remin_URDOMlifetime > loc_intI*dum_dtyr) then
                loc_bio_remin_URDOMratio = loc_intI*dum_dtyr/par_bio_remin_URDOMlifetime
             else
                loc_bio_remin_URDOMratio = 1.0
             end if
          else
             if (par_bio_remin_URDOMlifetime > dum_dtyr) then
                 loc_bio_remin_URDOMratio = dum_dtyr/par_bio_remin_URDOMlifetime
             else
                loc_bio_remin_URDOMratio = 1.0
             end if
          end if
      ...
      !   diag_production(idiag_production_LDOMrp,loc_i,loc_j,k) = - dum_vbio_remin%mk(conv_io_lselected(io_DOM_C),k) 
      !   diag_production(idiag_production_LDOMrp,loc_i,loc_j,k) = dum_vocn%mk(conv_io_lselected(io_DOM_C),k) - dum_vbio_remin%mk(conv_io_lselected(io_DOM_C),k) 
       end if
       ! RDOM
          ...
          if (dum_vocn%mk(conv_io_lselected(io_RDOM_C),k) > const_real_nullsmall) then
             DO l=3,n_l_ocn
                ...
                do loc_m=1,loc_tot_m
                   ...
                   if (ocn_select(io_URDOM_C)) then
                    loc_bio_part(is,k)  = loc_bio_part(is,k)  +  (conv_RDOM_POM(is,io)*loc_bio_remin_RDOMratio*dum_vocn%mk(l,k)) * 0.985
                    loc_vbio_remin(l,k) = loc_vbio_remin(l,k) - loc_bio_remin_RDOMratio*dum_vocn%mk(l,k)
                    lurdom=conv_io_lselected(io_URDOM_C)-conv_io_lselected(io_RDOM_C)
                    loc_vbio_remin(l+lurdom,k) = loc_vbio_remin(l+lurdom,k) + (loc_bio_remin_RDOMratio*dum_vocn%mk(l,k)) * 0.015
                   else
     ...
     !  diag_production(idiag_production_RDOMrp,loc_i,loc_j,k) = - dum_vbio_remin%mk(conv_io_lselected(io_RDOM_C),k) 
     !  diag_production(idiag_production_RDOMrp,loc_i,loc_j,k) = dum_vocn%mk(conv_io_lselected(io_RDOM_C),k) - dum_vbio_remin%mk(conv_io_lselected(io_RDOM_C),k) 
       end if
       ! URDOM 
       if (ocn_select(io_URDOM_C)) then 
          if (dum_vocn%mk(conv_io_lselected(io_URDOM_C),k) > const_real_nullsmall) then 
             DO l=3,n_l_ocn
                io = conv_iselected_io(l)
                loc_tot_m = conv_URDOM_POM_i(0,io)
                do loc_m=1,loc_tot_m
                   is = conv_URDOM_POM_i(loc_m,io)
                   loc_bio_part(is,k)  = loc_bio_part(is,k)  + conv_URDOM_POM(is,io)*loc_bio_remin_URDOMratio*dum_vocn%mk(l,k)
                   loc_vbio_remin(l,k) = loc_vbio_remin(l,k) - loc_bio_remin_URDOMratio*dum_vocn%mk(l,k)
                end do
             end do
          end if
      !   diag_production(idiag_production_URDOMrp,loc_i,loc_j,k) = - dum_vbio_remin%mk(conv_io_lselected(io_URDOM_C),k) - dum_vbio_remin%mk(conv_io_lselected(io_RDOM_C),k) * 0.015 
         diag_production(idiag_production_URDOMrp,loc_i,loc_j,k) =  - dum_vbio_remin%mk(conv_io_lselected(io_RDOM_C),k) * 0.015 
      !   diag_production(idiag_production_URDOMrp,loc_i,loc_j,k) = dum_vocn%mk(conv_io_lselected(io_URDOM_C),k) - dum_vbio_remin%mk(conv_io_lselected(io_URDOM_C),k) + dum_vbio_remin%mk(conv_io_lselected(io_RDOM_C),k) * 0.015 
       end if
