...
CONTAINS
  ...
  SUBROUTINE sub_save_netcdf_2d(dum_dtyr)
    ...
    If (ctrl_data_save_slice_diag_redox_old) then
       ...
       DO id=1,n_diag_production 
          loc_ij(:,:) = const_real_null
          DO i=1,n_i
             DO j=1,n_j
                If (goldstein_k1(i,j) <= n_k) then
                   loc_ij(i,j) = &
                        & sum(phys_ocn(ipo_M,i,j,:)*int_diag_production_timeslice(id,i,j,:))* &
                        & phys_ocn(ipo_rA,i,j,n_k)/int_t_timeslice
                end If
             end DO
          end DO  
          call sub_adddef_netcdf(loc_iou,3,'diag_int_'//trim(string_diag_production(id)), &
               & 'water-column integrated production/oxidation rate - '//trim(string_diag_production(id)), &
               & trim(loc_unitsname),const_real_zero,const_real_zero)
          call sub_putvar2d('diag_int_'//trim(string_diag_production(id)),loc_iou, &
               & n_i,n_j,loc_ntrec,loc_ij(:,:),loc_mask_surf)
       end DO
    end If    
